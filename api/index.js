...
module.exports = async function handler(req, res) {
  ...
  // Se Vercel aggiunge ?path=, usalo come url, altrimenti usa il path di req.url
  const pathParam = p.get('path') || '';
  let url;
  if (pathParam) {
    // pathParam può essere "strava/streams" o "strava" (da /auth/strava o /api/strava/...)
    // rawUrl ci dice il prefisso corretto: /api/... o /auth/...
  ...
  if (url.startsWith('/auth/strava') && !url.startsWith('/auth/strava/callback')) {
    ...
    const dest = 'https://www.strava.com/oauth/authorize?client_id='+CLIENT_ID+'&response_type=code&redirect_uri='+encodeURIComponent(BASE_URL+'/auth/strava/callback')+'&scope=read,activity:read_all&state='+uid;
    res.setHeader('Location', dest);
    return res.status(302).end();
  }

  if (url.startsWith('/auth/strava/callback')) {
    const code = p.get('code'), state = p.get('state') || 'default';
